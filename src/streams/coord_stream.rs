    ...
    QuadNum<P>: SylowDecomposable<S>,
{
    pub fn new(
        ...
        let hyper_stream = 
            FactorStream::new(FpNum::FACTORS.prime_powers(), limit, true)
                .into_iter()
                .fold(
                ...
                .into_iter();
        let ellip_stream =
            FactorStream::new(QuadNum::FACTORS.prime_powers(), limit, true)
                .into_iter()
                .fold(
    ...
    pub fn upper_triangle(self) -> impl ParallelIterator<
        Item = (Coord<P>, Coord<P>)
    > + 'a
    where
        S: Clone + Send + Sync,
    {
        ...
        impl<I: Iterator + Clone> Iterator for Helper<I> {
            ...
            fn next(&mut self) -> Option<Self::Item> {
                ...
                let Some(a) = self.0.next() else { return None; };
                Some((iter, a))
            }
        }
        Helper(self)
            .par_bridge()
            .flat_map(|(iter, a)| {
                ParallelIterator::map(iter, move |b| (a.clone(), b))
            })
    }
}
...
impl<
    ...
    const L_ELLIP: usize,
    const P: u128,
> Iterator for CoordStream<'a, S, L_HYPER, L_ELLIP, P> 
where
    FpNum<P>: SylowDecomposable<S>,
    QuadNum<P>: SylowDecomposable<S>,
{
...
impl<
    ...
    const L_ELLIP: usize,
    const P: u128,
> ParallelIterator for CoordStream<'a, S, L_HYPER, L_ELLIP, P>
where
    S: Send + Sync,
    FpNum<P>: SylowDecomposable<S>,
    QuadNum<P>: SylowDecomposable<S>,
